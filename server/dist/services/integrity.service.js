...
class IntegrityService {
    calculateIntegrityScore(factors) {
        let score = 100;
    ...
    async detectTimingAnomalies(sessionId) {
    ...
    async generateExamIntegrityReport(examId) {
        ...
        for (const enrollment of enrollments) {
            ...
            const factors = {
                ...
                proctorFlagSeveritySum: enrollment.session.flags.reduce((s, f) => s + f.severity, 0),
                timingAnomalyCount: enrollment.session.flags.filter((f) => f.flagType === "TIMING_ANOMALY").length,
    ...
    async detectCollusion(examId, enrollments) {
        const candidateScores = new Map();
        const candidateAnswers = new Map();
        for (const enrollment of enrollments) {
            if (!enrollment.session)
                continue;
            candidateAnswers.set(enrollment.candidateId, enrollment.session.answers.map((a) => ({
        ...
        for (let i = 0; i < candidateIds.length; i++) {
            for (let j = i + 1; j < candidateIds.length; j++) {
                ...
                const similarity = similarity_service_1.similarityService.compareAnswerPatterns(answers1, answers2);
