...
exports.adaptiveEngineService = exports.AdaptiveEngineService = void 0;
const database_config_1 = __importDefault(require("../config/database.config"));
class AdaptiveEngineService {
    async getNextQuestion(sessionId, examId, adaptiveState) {
        ...
        const allPastAnsweredQuestionIds = new Set((await database_config_1.default.candidateAnswer.findMany({
            where: {
                session: {
                    enrollment: {
                        candidateId,
        ...
        const answeredInCurrentSession = new Set((await database_config_1.default.candidateAnswer.findMany({
            where: { sessionId },
            select: { examQuestionId: true },
        })).map((a) => a.examQuestionId));
        const unanswered = examQuestions.filter((eq) => !allPastAnsweredQuestionIds.has(eq.id));
        if (unanswered.length === 0)
            return null;
        const topicAnswerCounts = {};
        ...
        const abilityTargetDifficulty = this.calculateTargetDifficulty(adaptiveState);
        if (exam.adaptiveConfig) {
            ...
            const idealPerQuestion = remainingQuestions > 0
            ...
            const blendedTargetDifficulty = weightAbility * abilityTargetDifficulty +
            ...
            const zeroTopics = topics.filter((topic) => (topicAnswerCounts[topic] || 0) === 0);
            ...
            for (const eq of topicQuestions) {
                ...
                if (score < bestScore) {
                    ...
                    const bestFinalDiff = Math.abs(sumDifficultySoFar +
                        selected.questionVersion.difficulty -
                        targetDifficultySum);
                    const candidateFinalDiff = Math.abs(sumDifficultySoFar + difficulty - targetDifficultySum);
            ...
            return {
                ...
                screenReaderHint: this.generateScreenReaderHint(selected.question.type, version.content),
            };
        }
        const topicGroups = new Map();
        for (const eq of unanswered) {
        ...
        const sorted = topicQuestions.sort((a, b) => {
            const diffA = Math.abs(a.questionVersion.difficulty - abilityTargetDifficulty);
            const diffB = Math.abs(b.questionVersion.difficulty - abilityTargetDifficulty);
            return diffA - diffB;
        });
    ...
    async getNextQuestionSequential(sessionId, examId, currentIndex) {
        const session = await database_config_1.default.examSession.findUnique({
            where: { id: sessionId },
            include: { enrollment: true },
        });
        if (!session)
            return null;
        const candidateId = session.enrollment.candidateId;
        const pastAnsweredQuestionIds = new Set((await database_config_1.default.candidateAnswer.findMany({
            where: {
        ...
        const examQuestion = await database_config_1.default.examQuestion.findFirst({
            where: {
                examId,
        ...
        return {
            ...
            screenReaderHint: this.generateScreenReaderHint(examQuestion.question.type, version.content),
        };
    }
    calculateTargetDifficulty(state) {
        if (state.questionsServed === 0)
            return 5;
        const overallAccuracy = state.questionsServed > 0 ? state.runningAccuracy : 0.5;
        let adjustment = 0;
        if (overallAccuracy >= 0.8)
            adjustment = 2;
        else if (overallAccuracy >= 0.6)
            adjustment = 1;
        else if (overallAccuracy <= 0.2)
            adjustment = -2;
    ...
    updateAdaptiveState(state, topic, isCorrect, questionDifficulty) {
        ...
        const totalCorrect = Object.values(newState.topicAccuracyMap).reduce((sum, t) => sum + t.correct, 0);
        const totalAnswered = Object.values(newState.topicAccuracyMap).reduce((sum, t) => sum + t.total, 0);
    ...
    generateScreenReaderHint(type, content) {
        const typeLabel = {
...
exports.adaptiveEngineService = new AdaptiveEngineService();
//# sourceMappingURL=adaptive-engine.service.js.map